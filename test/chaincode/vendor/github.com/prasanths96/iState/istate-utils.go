...
//
func (iState *iState) encodeState(oMap map[string]interface{}, keyref string, isQuery ...bool) (encodedKeyValPairs map[string]string, docsCounter map[string]int, encKeyDocNameMap map[string]string, iStateErr Error) {
	...
	encodedKeyValPairs = make(map[string]string)
	...
	for i := 0; i < len(relationalTables); i++ {
		for j := 0; j < len(relationalTables[i]); j++ {
			...
			if docType, ok := relationalTables[i][j][docTypeField]; ok {
				...
				encodedKey = joinStringInterfaceSlice(encodedKeyParts, seperator) + seperator + encodedVal
				...
				default:
					...
					encodedKeyValPairs[encodedKey] = keyref
				}
			}
...
func initQueryEnv(qEnv *queryEnv) {
	...
	qEnv.ukeyEncKVMap = make(map[string]map[string]string)
}
