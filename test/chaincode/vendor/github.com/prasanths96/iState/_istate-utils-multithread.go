...
type rTablesP struct {
	mux    sync.Mutex
	tables *[][]map[string]interface{}
}

func newrTablesP() (rTP *rTablesP) {
	rTP = &rTablesP{
		tables: &[][]map[string]interface{}{},
	}
	return
...
func (rTP *rTablesP) append(newTable []map[string]interface{}) {
	rTP.mux.Lock()
	defer rTP.mux.Unlock()

	*rTP.tables = append(*rTP.tables, newTable)
}
...
func (iState *iState) generateRelationalTables(obj map[string]interface{}, keyref string, isQuery bool) (relationalTables [][]map[string]interface{}, iStateErr Error) {
	...
	indexes := make([]string, len(obj), len(obj))
	i := 0
	for index := range obj {
		indexes[i] = index
		i++
	...
	numParts := len(indexes) / GOMAXPROCS
	extra := len(indexes) % GOMAXPROCS
	var mux1 sync.Mutex
	var mux2 sync.Mutex
	for j := 0; j < GOMAXPROCS; j++ {
		condition := (numParts * (j + 1))
		if j == GOMAXPROCS-1 {
			condition = condition + extra
		}
		wg.Add(1)
		go func(numParts int, condition int, indexes []string) {
			for i := j * numParts; i < condition; i++ {
				mux1.Lock()
				index := indexes[i]
				mux1.Unlock()
				mux2.Lock()
			...
			wg.Done()
		}(numParts, condition, indexes)
		fmt.Println("Num GoRoutines Active: ", runtime.NumGoroutine())
	}
	wg.Wait()
	relationalTables = *rTP.tables
	return
}
