...
func (qEnv *queryEnv) readufetchedKVMap(key string) (valBytes []byte, ok bool) {
	qEnv.mux.Lock()
	defer qEnv.mux.Unlock()
	valBytes, ok = qEnv.ufetchedKVMap[key]
	return
}

func (qEnv *queryEnv) addufetchedKVMap(key string, valBytes []byte) {
	qEnv.mux.Lock()
	defer qEnv.mux.Unlock()
	qEnv.ufetchedKVMap[key] = valBytes
	return
}
...
func addToTree(tree *btree.Tree, encKey string, genericField string, numDocs int, fetchFunc func(shim.ChaincodeStubInterface, string, *queryEnv) (map[string][]byte, Error), relatedEncKV map[string][]byte) {
